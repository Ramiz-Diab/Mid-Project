...
export default function BookingForms() {
  ...
  useEffect(() => {
    async function fetchService() {
      //fetchService() tries two Firestore collections in sequence: "Collection"
      //             then "Users". This is two separate Firestore reads every time the page loads.
      //             This is a design smell — bookable services and user accounts should not
      //             live in different collections and be fetched with fallback logic like this.
      //             Normalize your data model so services always come from one collection.
      try {
        setError("");
        setService(null);
        ...
        if (userSnap.exists()) {
          ...
          setService({
            ...
            isAvailable:
              typeof u.isAvailable === "boolean" ? u.isAvailable : true,
          });
          return;
        }
